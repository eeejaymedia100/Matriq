...
**OCR — hybrid accuracy pass IMPLEMENTED (this round):**
- Reality check that drove it: committed OCR was **Tesseract-only** since `f63c17d` (the Gemini-first OCR of the 08-16 entries was the source of the "OCR not working" complaints and was reverted). The user's pain: **Tesseract is very bad at handwriting** — its LSTM is trained on printed text, so no preprocessing tuning can fix that.
- **Shipped the fix (committed with this FCM round's session):** `tools.service.ts` now runs a **two-tier pipeline** — 1) Tesseract first (free/fast, unchanged on clean printed text), 2) if Tesseract confidence < **55%** (the classic handwriting / low-light / noisy-photo failure) it escalates to a **Gemini vision pass**: image downscaled to ≤2048px + JPEG re-encode (the big latency lever), verbatim-transcription prompt engineered for handwriting (course codes, abbreviations, line breaks preserved; `NO_TEXT` marker when there's nothing legible), 60s timeout with one retry on transient 5xx, env-gated on the already-wired `GEMINI_API_KEY`. No key → exact old behaviour. `OcrResult.engine` is now `"tesseract" | "gemini"`; mobile doesn't display the engine so no app change needed.
- Verified: **207 tests green** (22 suites — 4 new hybrid-pipeline specs: trust-high-confidence, handwriting→Gemini rescue, no-key keeps Tesseract, Gemini-blank keeps Tesseract), backend tsc clean.
- **Cost behaviour is inverted vs the old Gemini-first design:** printed scans cost $0 (Tesseract), only the mangled ~10–15% hit the paid API — that's the accuracy fix without the old 503/latency/cost problems.
- Still on the shelf (not this round): OCR concurrency semaphore + result cache, `--psm 6` preprocessing variants, client-side pre-compress.

**Next (honest):** 1) the Firebase runbook above is blocked on the user (Firebase project + the two credentials). 2) The OCR rescue ships with the next backend deploy — needs a real-image pass on the VM (handwritten note vs printed page vs blank page) to confirm quality + latency on live Gemini.
